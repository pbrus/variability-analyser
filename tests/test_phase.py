...
class PhaseTest(unittest.TestCase):
    ...
    def test_read_lightcurve(self):
        results = np.array([
            [1.65623458e+02, 1.65635605e+02, 1.65651111e+02],
    ...
    def test_prepare_data_get_model(self):
        phase = np.array([
            0., 0.18512029, 0.42143173, 0.01751698, 0.28033702,
            0.76573414, 0.19333808, 0.97463194, 0.65582669, 0.39205587
        ])
        magnitude = np.array([
            -0.04774, -0.36772, 0.10621, -0.15934, -0.28908,
            0.02377, -0.38645, -0.06013, 0.26934, 0.07838
        ])
        X = np.array([
            0., 0.01005025, 0.0201005, 0.03015075, 0.04020101,
            0.05025126, 0.06030151, 0.07035176, 0.08040201, 0.09045226
        ])
        Y = np.array([
            -0.11475255, -0.13069089, -0.14593987, -0.16038649, -0.17394149,
            -0.1865397, -0.19813953, -0.20872167, -0.21828688, -0.22685327
        ])
        ...
        self.assertTrue(np.allclose(x_model[:10], X))
        self.assertTrue(np.allclose(y_model[:10], Y))
    ...
    def test_sines_sum(self):
        ...
        frequency = 1.364
        results = np.array([
            17.648670550049133, 17.000776679204602, 16.995637267938356,
            17.70303244205788, 18.39903676848221, 17.870908510716415,
            17.4000763292155, 17.238202876538708, 18.217723680624406
        ])
