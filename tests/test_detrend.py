"""
Test package of the varana.detrend module
"""
import unittest
import numpy as np
...
class DetrendTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.x_domain = np.array([
            165.445448, 166.71958418, 167.99372036, 169.26785654, 170.54199272,
            171.8161289, 173.09026508, 174.36440126, 175.63853744, 176.91267362
        ])
    ...
    def test_calculate_kmeans(self):
        clusters_centers = np.array([
            [9.95274508e+02, -9.85392445e-02],
            [2.67177517e+02,  2.65203604e-01],
            [1.35113653e+03,  6.14042550e-02],
            [6.29255809e+02, -6.68374040e-02]
        ])
    ...
    def test_spline_function(self):
        spline_coeffs = np.array([
            -0.37832, 2.81363646, 1.25721844, -1.46388609, 3.21396524,
            -2.60229307, 4.20855241, -0.65726299, 1.93272306, -1.44624978
        ])
    ...
    def test_x_domain_spline(self):
        self.assertTrue(np.allclose(self.x_domain,
                                    x_domain_spline(self.data[:, 0])[:10]))
    ...
    def test_y_domain_spline(self):
        ...
        self.assertTrue(np.allclose(func(self.x_domain),
                                    y_domain_spline(func, self.x_domain)))
    ...
    def test_detrend_data(self):
        results = np.array([
            [1.65445448e+02, 2.21650434e+01, 6.70000000e-04],
            [1.66264696e+02, 2.42829409e+01, 7.20000000e-04],
            [1.66947026e+02, 2.31119208e+01, 5.10000000e-04],
            [1.67665380e+02, 2.21544714e+01, 1.08000000e-03],
            [1.68527654e+02, 2.39932796e+01, 8.50000000e-04],
            [1.69521903e+02, 2.24496271e+01, 8.40000000e-04],
            [1.70285055e+02, 2.52439317e+01, 9.80000000e-04],
            [1.70848836e+02, 2.35367251e+01, 8.20000000e-04],
            [1.71769190e+02, 2.35426657e+01, 5.20000000e-04],
            [1.72479518e+02, 2.17447055e+01, 5.90000000e-04]
        ])
